...
ExecuteResult execute_insert(Statement *statement, Table *table) {
  ...
  Row *rows_to_insert = &(statement->row_to_insert);

  serialize_row(rows_to_insert, row_slot(table, table->num_rows));
  table->num_rows++;
...
void *row_slot(Table *table, uint32_t row_num) {
  ...
  if (page == NULL) {
    // Allocate memory only when we try to acdess the page
